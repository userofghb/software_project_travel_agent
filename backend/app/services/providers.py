...
def _build_meal_activity(time: str, period: str, city: str, budget_range: str) -> dict[str, Any]:
    ...
    return {
        "time": time,
        "period": period,
        "title": "午餐与休息" if is_lunch else "晚餐与自由活动",
        "type": "food",
        "poi_id": None,
        "location": None,
        ...
        "tags": ["餐饮", "休息"] if is_lunch else ["餐饮", "自由活动"],
    }
    ...
    def generate_plan(
        ...
        context = {
            ...
            "planning_rules": {
                ...
                "route_agent": "不要把从酒店出发、景点之间转场、公交地铁怎么走写成景点 activity；Route Agent 会在后续调用高德路线 API 并插入 type=transport 的转场方案。",
                "hotel_display": "不要省略酒店住宿；住宿会在后续归一化为前一晚 evening 的 type=hotel 活动。",
                "meal_copy": "餐饮标题不要写“北京本地午餐/上海本地晚餐”这类城市占位文案，可写“午餐与休息”“晚餐与自由活动”或具体餐厅建议。",
...
def _normalize_activity_type(raw_type: Any, title: Any, reason: Any) -> str:
    ...
    if any(token in value for token in ("food", "meal", "lunch", "dinner", "餐")):
        return "food"
...
def _needs_activity_completion(activities: list[dict[str, Any]]) -> bool:
    ...
    has_food = any(activity.get("type") == "food" for activity in core_activities)
    return len(core_activities) < 4 or not {"morning", "afternoon", "evening"}.issubset(periods) or not has_food
...
def _find_restaurant_for_meal(city: str, activities: list[dict[str, Any]], index: int) -> dict[str, Any]:
    ...
    return _mock_restaurant(city, anchor_location, index)
...
def _meal_activity_from_restaurant(
    ...
    profile = _budget_profile(budget_range)
    period = activity.get("period") or _period_from_time(activity.get("time"))
    is_lunch = period == "lunch"
    title = restaurant.get("name") or ("午餐餐馆" if is_lunch else "晚餐餐馆")
    return {
        **activity,
        "title": title,
        "type": "food",
        ...
        "budget": activity.get("budget") if activity.get("budget") is not None else (profile["lunch"] if is_lunch else profile["dinner"]),
        "tags": _restaurant_tags(restaurant, is_lunch),
    }


def _restaurant_tags(restaurant: dict[str, Any], is_lunch: bool) -> list[str]:
    tags = ["午餐" if is_lunch else "晚餐", "餐馆"]
...
def _mock_restaurant(city: str, anchor_location: dict[str, float] | None, index: int) -> dict[str, Any]:
...
FOOD_PLACEHOLDER_TOKENS = ("本地午餐", "当地午餐", "特色午餐", "午餐与休息", "午餐", "本地晚餐", "当地晚餐", "晚餐与自由活动", "晚餐")
...
def validate_and_repair_plan(
    ...
    repaired["map"] = {"points": build_map_points_from_nodes(normalized_days), "routes": []}
...
def _find_restaurant_between_nodes(city: str, nodes: list[dict[str, Any]], index: int, used_ids: set[str]) -> dict[str, Any] | None:
    ...
    if use_real_amap:
        try:
            pois = _amap_place_around(city=city, location=anchor, keywords=f"{city} 餐厅 美食", types="050000", offset=12)
            ranked = sorted(
            ...
            for poi in ranked:
                ...
                if poi.get("location") and poi_id not in used_ids:
                    return poi
        except Exception:
            pass
    return _mock_restaurant(city, anchor, index)
...
def _add_intercity_nodes(
    ...
    if day_index == total_days - 1:
        ...
        intercity = _intercity_node(trip, "return", return_terminal, departure_start, departure_end)
...
def _terminal_poi(city: str, mode: str, destination: bool, warnings: list[str]) -> dict[str, Any]:
    ...
    try:
        ...
        location = None
    location = location or {"lng": 121.47, "lat": 31.23}
...
def _transport_node_between(previous: dict[str, Any], next_node: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    duration_minutes = max(8, int(distance_km / (28 if mode == "driving" else 18) * 60) + 8)
    ...
    return {
        ...
        "type": "transport",
        "title": f"{previous.get('title') or '上一站'} → {next_node.get('title') or '下一站'}",
        "start_time": _format_minutes(start),
        "end_time": _format_minutes(end),
...
def _repair_node_times(nodes: list[dict[str, Any]], day_index: int, total_days: int) -> list[dict[str, Any]]:
    ...
    return_start = latest_end - return_duration if return_node else latest_end
    ...
    for node in sorted(nodes, key=lambda item: _time_text_minutes(str(item.get("start_time") or "23:59"))):
        ...
        if _is_dinner_node(node):
            duration = min(duration, 90)
        current_start = _time_text_minutes(str(node.get("start_time") or ""))
        ...
        if node.get("type") not in {"intercity_transport", "hotel"} and not _is_dinner_node(node) and start >= regular_latest_end:
            continue
        if node.get("type") not in {"intercity_transport", "hotel"} and not _is_dinner_node(node) and end > regular_latest_end:
            duration = max(30, regular_latest_end - start)
            end = start + duration
        if _is_dinner_node(node) and start > dinner_start_deadline:
            continue
        if _is_dinner_node(node) and return_node and end > return_start - 10:
        ...
        if start >= latest_end or end > latest_end + 1:
            continue
        node = dict(node)
